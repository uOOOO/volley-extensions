...
public class GsonRequestIntegrationTest {
	...
	public void responseShouldBeParsedWithSpecialChars() throws Exception {
		...
		// When
		Request<Person> request = new GsonRequest<>(url, Person.class, listener, errorListener);
	...
	public void responseShouldBeParsedWithUnknownProperties() throws Exception {
		...
		// When
		Request<Person> request = new GsonRequest<Person>(url,
	...
	public void errorShouldBeListenedWhenParseInvalidFormat() throws Exception {
		...
		// When
		Request<Person> request = new GsonRequest<>(url,
				Person.class, listener, errorListener);
		requestQueue.add(request);
		...
		assertThat(error.getCause(), is(instanceOf(JsonSyntaxException.class)));
	}
